...
def build_sft_trainer(model, tokenizer, dataset, training_args, max_seq_length=256):
    ...
    # Goal: wire model, tokenizer, dataset, and training_args into an SFTTrainer
...
def switch_to_inference_mode(model):
    ...
    # Goal: call the Unsloth helper that prepares the model for fast generation
    return flm.for_inference(model)
...
def build_chat_prompt(tokenizer, instruction):
    ...
    # Goal: wrap the instruction as a user turn and produce the assistant-generation prompt string
    
    messages = [
        {"role": "user", "content": instruction}
    ]
    
    # Apply the chat template to format the string
    # tokenize=False returns a string instead of token IDs
    # add_generation_prompt=True appends the assistant's starting token(s)
...
def generate_reply(model, tokenizer, prompt, max_new_tokens=32):
    ...
    # Goal: tokenize prompt, run model.generate with do_sample=False, decode new tokens only
    
    # 1. Tokenize and move inputs to the model's device
    inputs = tokenizer(prompt, return_tensors = 'pt' ).to(model.device) # Shape: (1, T)

    # 2. Generate token IDs (passing the tokenized inputs, not the string)
    output_ids = model.generate(
        **inputs, 
        max_new_tokens=max_new_tokens, 
        do_sample=False)

    # 3. Slice the output tensor to isolate only the newly generated tokens
